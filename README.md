...
- 独立行为 fixture：6/6 通过。
- 已适配：共享 `MEDIA_TAG_RE`、Windows drive `MEDIA:`、stream 显示清理、native image 本地路径不泄漏、forced native/image routing、busy queue/send_message Windows MEDIA、`/retry` 媒体历史拒绝、outbound audit。

结论：本包在老白本机 Hermes v0.15.1 上等级 B，可用但还缺真实飞书 PC/手机手工样例验收，暂不标 A。
